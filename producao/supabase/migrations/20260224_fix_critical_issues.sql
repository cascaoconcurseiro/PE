...
DO $
BEGIN
    -- Desabilitar trg_sync_ddd_ledger se existir
    IF EXISTS (
    ...
    -- Desabilitar sync_transaction_to_ddd_ledger se existir
    IF EXISTS (
        SELECT 1 FROM pg_trigger 
        WHERE tgname = 'sync_transaction_to_ddd_ledger' 
        AND tgrelid = 'transactions'::regclass
    ) THEN
        ALTER TABLE transactions DISABLE TRIGGER sync_transaction_to_ddd_ledger;
    END IF;
END $;
    ...
    SELECT 
        ...
        COALESCE(SUM(CASE 
            WHEN t.type = 'DESPESA' THEN 
                -- Se eu paguei e compartilhei, minha despesa = total - valor compartilhado
                CASE 
                    ...
                    -- Se outro pagou e eu devo, minha despesa = meu valor atribuído
...
BEGIN
    -- Calcular Receivables (A Receber)
    -- Transações onde EU paguei e outros devem para mim
    SELECT 
        COALESCE(SUM(
    ...
    -- Calcular Payables (A Pagar)
    -- Transações onde OUTRO pagou e eu devo
    SELECT 
...
-- ==============================================================================
-- PARTE 6: ATUALIZAR FUNÇÃO CREATE_SHARED_TRANSACTION_V2 PARA INCLUIR NOTES
-- ==============================================================================
