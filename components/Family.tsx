...
export const Family: React.FC<FamilyProps> = ({ members, onAddMember, onDeleteMember }) => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div>
                <h2 className="text-2xl font-bold text-slate-800">Minha Família</h2>
                <p className="text-slate-600 text-sm">Gerencie os membros para dividir despesas.</p>
            </div>
            ...
            <Card title="Adicionar Membro">
                <form onSubmit={handleSubmit} className="flex gap-4 items-end">
                    ...
                    <div className="flex-1">
                        <label className="block text-sm font-bold text-slate-700 mb-1">Parentesco (Opcional)</label>
                        <input 
                            className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 placeholder:text-slate-500"
                            value={role}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {members.length === 0 && (
                    <div className="col-span-2 text-center py-8 text-slate-500 bg-slate-50 border border-dashed rounded-xl border-slate-300">
                        Nenhum membro cadastrado. Adicione alguém para começar a compartilhar gastos.
                    </div>
                )}
                {members.map(member => (
                    <div key={member.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex justify-between items-center hover:shadow-md transition-shadow">
