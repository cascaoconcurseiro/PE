...
import {
    ...
    ChevronLeft, ChevronRight, Bell, BellRing, AlertTriangle, Sparkles, Menu, Home, FileText, Plus
} from 'lucide-react';
...
export const MainLayout: React.FC<MainLayoutProps> = ({
    ...
    const SidebarItem = ({ view, icon: Icon, label, activeBg, activeText }: { view: View, icon: any, label: string, activeBg: string, activeText: string }) => {
        ...
        return (
            <button
                ...
                className={`w-full text-left px-4 py-3.5 rounded-xl flex items-center gap-4 text-sm font-semibold transition-all duration-200 ${isActive ? `${activeBg} ${activeText} shadow-sm` : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200'}`}
            >
                <div className={`p-2 rounded-lg transition-colors ${isActive ? 'bg-white/50 dark:bg-white/10' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
    ...
    const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => (
        <div className="flex flex-col h-full bg-white dark:bg-slate-900">
            {isMobile && (
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-gradient-to-br from-slate-50 to-slate-100/50 dark:from-slate-900 dark:to-slate-800 shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-bold text-xl shadow-sm border border-white dark:border-slate-700">
                            {user.name.charAt(0)}
                        </div>
                        <div className="overflow-hidden">
                            <h3 className="font-bold text-slate-900 dark:text-white text-base truncate max-w-[160px]">{user.name}</h3>
                            <p className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[160px]">{user.email}</p>
                        </div>
                    </div>
                    <button onClick={() => setIsMenuOpen(false)} className="p-2 bg-white dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                <div className="space-y-1 mb-6">
                    ...
                    <SidebarItem view={View.DASHBOARD} icon={LayoutDashboard} label="Visão Geral" activeBg="bg-emerald-50 dark:bg-emerald-900/20" activeText="text-emerald-700 dark:text-emerald-400" />
                    ...
                    <SidebarItem view={View.ACCOUNTS} icon={Wallet} label="Contas" activeBg="bg-violet-50 dark:bg-violet-900/20" activeText="text-violet-700 dark:text-violet-400" />
                ...
                <div className="space-y-1">
                    ...
                    <button
                        ...
                        className="w-full text-left px-4 py-3.5 rounded-xl flex items-center gap-4 text-sm font-semibold transition-all duration-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200"
                    >
                        <div className={`p-2 rounded-lg transition-colors ${!showValues ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 pb-safe bg-slate-50/50 dark:bg-slate-900/50 shrink-0 space-y-2">
                <button onClick={toggleTheme} className="w-full py-3.5 flex items-center justify-center gap-2 text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl font-bold transition-all active:scale-95 shadow-sm">
                    {theme === 'dark' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
                    {theme === 'dark' ? 'Modo Claro' : 'Modo Escuro'}
                </button>
                <button onClick={onLogout} className="w-full py-3.5 flex items-center justify-center gap-2 text-red-600 bg-white dark:bg-slate-800 border border-red-100 dark:border-red-900/30 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl font-bold transition-all active:scale-95 shadow-sm">
        ...
        return (
            ...
            >
                <div className={`p-2 rounded-2xl transition-all duration-300 ${isActive ? `${activeColor.replace('text-', 'bg-')}/10` : 'bg-transparent'}`}>
                    <Icon
                        className={`w-6 h-6 transition-colors duration-300 ${isActive ? activeColor : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`}
                        strokeWidth={isActive ? 2.5 : 2}
                    />
                </div>
                <span className={`text-[10px] font-bold mt-1 transition-colors duration-300 ${isActive ? activeColor : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`}>
                    {label}
                </span>
            </button>
        );
