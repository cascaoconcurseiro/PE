        ...
        <div className="flex flex-col h-screen max-h-[100dvh] bg-white dark:bg-slate-900 relative overflow-hidden">
            ...
            <div className="sticky top-0 px-2 py-1 shrink-0 border-b border-slate-100 dark:border-slate-700 flex items-center gap-1.5 bg-white dark:bg-slate-900 z-30">
                <div className="flex bg-slate-100 dark:bg-slate-800 p-0.5 rounded-lg relative shadow-inner flex-1">
                    <button onClick={() => setFormMode(TransactionType.EXPENSE)} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md transition-all ${isExpense ? 'bg-white dark:bg-slate-700 text-red-700 dark:text-red-400 shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'}`}>
                        <ArrowDownLeft className="w-3.5 h-3.5" />
                        <span className="text-xs font-bold">Despesa</span>
                    </button>
                    <button onClick={() => setFormMode(TransactionType.INCOME)} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md transition-all ${isIncome ? 'bg-white dark:bg-slate-700 text-emerald-700 dark:text-emerald-400 shadow-sm' : 'text-slate-600 dark:text-slate-400'}`}>
                        <ArrowUpRight className="w-3.5 h-3.5" />
                        <span className="text-xs font-bold">Receita</span>
                    </button>
                    <button onClick={() => setFormMode(TransactionType.TRANSFER)} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md transition-all ${isTransfer ? 'bg-white dark:bg-slate-700 text-blue-700 dark:text-blue-400 shadow-sm' : 'text-slate-600 dark:text-slate-400'}`}>
