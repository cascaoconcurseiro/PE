    ...
    if (selectedTrip) {
        ...
        return (
            <div className="space-y-6 animate-in fade-in zoom-in duration-300 pb-24">
                ...
                {activeTab === 'OVERVIEW' && (
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                        ...
                        <Card className="border-violet-100 dark:border-violet-900">
                            <div className="flex justify-between items-center mb-4"><div className="flex items-center gap-2 text-violet-800 dark:text-violet-300"><Target className="w-5 h-5" /><h3 className="font-bold">Orçamento da Viagem</h3></div>{!isEditingBudget ? (<button onClick={startEditingBudget} className="p-2 text-slate-400 hover:text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-900/30 rounded-full transition-colors no-print"><Pencil className="w-4 h-4" /></button>) : (<div className="flex gap-2"><button onClick={() => setIsEditingBudget(false)} className="p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full"><X className="w-4 h-4" /></button><button onClick={handleSaveBudget} className="p-2 text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 rounded-full"><Save className="w-4 h-4" /></button></div>)}</div>
                            {isEditingBudget ? (<div className="mb-4"><label className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase">Definir Limite Total</label><input type="number" autoFocus className="w-full text-2xl font-bold text-slate-800 dark:text-white border-b-2 border-violet-200 dark:border-violet-700 outline-none py-1 focus:border-violet-500 bg-transparent" value={tempBudget} onChange={e => setTempBudget(e.target.value)} placeholder="0,00" /></div>) : (<><div className="mb-3"><div className="flex justify-between text-sm font-medium mb-1"><span className={`${isOverBudget ? 'text-red-700 dark:text-red-400' : 'text-slate-700 dark:text-slate-200'}`}>{formatCurrency(totalSpent, selectedTrip.currency)} ({percentUsed.toFixed(0)}%)</span><span className="text-slate-500 dark:text-slate-400">Meta: {formatCurrency(budget, selectedTrip.currency)}</span></div><div className="h-3 w-full bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all duration-1000 ${percentUsed > 100 ? 'bg-red-600' : percentUsed > 75 ? 'bg-amber-500' : 'bg-emerald-600'}`} style={{ width: `${Math.min(percentUsed, 100)}%` }}></div></div></div>{budget > 0 && (<div className={`text-xs font-bold p-2 rounded-lg text-center ${isOverBudget ? 'bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-300' : 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-300'}`}>{isOverBudget ? `Orçamento excedido em ${formatCurrency(Math.abs(remaining), selectedTrip.currency)}` : `Resta ${formatCurrency(remaining, selectedTrip.currency)} disponível`}</div>)}{budget === 0 && (<div className="text-xs text-center text-slate-500 dark:text-slate-400 italic">Defina um orçamento para acompanhar seu progresso.</div>)}</>)}
                        </Card>
                ...
                {/* TAB 4: STATS */}
                {activeTab === 'STATS' && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300"><Card title="Gastos por Categoria">{tripTransactions.length > 0 ? (<div className="h-64"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={Object.entries(tripTransactions.reduce((acc, t) => { if (t.type === TransactionType.EXPENSE) acc[t.category] = (acc[t.category] || 0) + t.amount; return acc; }, {} as any)).map(([name, value]) => ({ name, value }))} dataKey="value" cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5}>{Object.entries(tripTransactions).map((_, index) => (<Cell key={`cell-${index}`} fill={['#8b5cf6', '#ec4899', '#f59e0b', '#10b981', '#3b82f6'][index % 5]} />))}</Pie><Tooltip formatter={(value: number) => formatCurrency(value, selectedTrip.currency)} /><Legend /></PieChart></ResponsiveContainer></div>) : (<div className="h-32 flex items-center justify-center text-slate-500 dark:text-slate-400 text-sm">Sem dados suficientes</div>)}</Card><div className="grid grid-cols-2 gap-4"><Card className="bg-violet-50 dark:bg-violet-900/20 border-violet-100 dark:border-violet-800"><p className="text-xs font-bold text-violet-600 dark:text-violet-400 uppercase">Média Diária</p><p className="text-2xl font-bold text-violet-800 dark:text-violet-300 mt-1">{formatCurrency(totalSpent / (Math.max(1, (new Date(selectedTrip.endDate).getTime() - new Date(selectedTrip.startDate).getTime()) / (1000 * 3600 * 24))), selectedTrip.currency)}</p></Card><Card className="bg-indigo-50 dark:bg-indigo-900/20 border-indigo-100 dark:border-indigo-800"><p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase">Total Itens</p><p className="text-2xl font-bold text-indigo-800 dark:text-indigo-300 mt-1">{tripTransactions.length}</p></Card></div></div>)}
