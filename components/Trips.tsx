            ...
            <div className="space-y-6 animate-in slide-in-from-bottom-10 duration-300">
                ...
                <Card className="bg-white border-violet-100 shadow-lg">
                    <form onSubmit={handleCreateOrUpdateTrip} className="space-y-6">
                        <div><label className="block text-sm font-bold text-slate-700 mb-1">Nome da Viagem</label><input className="w-full text-lg font-semibold text-slate-900 border-b-2 border-slate-200 focus:border-violet-500 outline-none py-2 bg-transparent placeholder-slate-500 transition-colors" placeholder="Ex: Férias em Miami" value={newTripName} onChange={e => setNewTripName(e.target.value)} autoFocus required /></div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="relative group"><div className="bg-white border border-slate-300 rounded-xl flex flex-col justify-center shadow-sm hover:border-violet-400 transition-all focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-100 relative h-20 overflow-hidden"><div className="absolute top-2 left-3 flex items-center gap-2 pointer-events-none"><Calendar className="w-4 h-4 text-violet-600" /><span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Início</span></div><input type="date" className="w-full h-full pt-6 px-3 text-center font-bold text-slate-900 bg-transparent border-none outline-none cursor-pointer" value={newStartDate} onClick={(e) => e.currentTarget.showPicker()} onChange={e => setNewStartDate(e.target.value)} required /></div></div>
                            <div className="relative group"><div className={`bg-white border rounded-xl flex flex-col justify-center shadow-sm transition-all relative h-20 overflow-hidden ${duration <= 0 ? 'border-red-300' : 'border-slate-300 hover:border-violet-400 focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-100'}`}><div className="absolute top-2 left-3 flex items-center gap-2 pointer-events-none"><Calendar className={`w-4 h-4 ${duration <= 0 ? 'text-red-600' : 'text-violet-600'}`} /><span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Fim</span></div><input type="date" className="w-full h-full pt-6 px-3 text-center font-bold text-slate-900 bg-transparent border-none outline-none cursor-pointer" value={newEndDate} onClick={(e) => e.currentTarget.showPicker()} onChange={e => setNewEndDate(e.target.value)} min={newStartDate} required /></div></div>
                        </div>
                        ...
                        <div><label className="block text-sm font-bold text-slate-700 mb-2">Moeda Principal</label><div className="relative"><Globe className="absolute left-3 top-3 w-5 h-5 text-slate-400" /><select value={newTripCurrency} onChange={(e) => setNewTripCurrency(e.target.value)} className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-300 bg-white text-slate-900 font-bold outline-none focus:ring-2 focus:ring-violet-500 appearance-none">{AVAILABLE_CURRENCIES.map(curr => (<option key={curr.code} value={curr.code}>{curr.code} - {curr.name}</option>))}</select><ChevronDown className="absolute right-3 top-3.5 w-4 h-4 text-slate-400 pointer-events-none" /></div></div>
                        <div><label className="block text-sm font-bold text-slate-700 mb-3 flex items-center gap-2"><Users className="w-4 h-4 text-violet-700" /> Quem vai?</label>{familyMembers.length === 0 ? (<div className="p-4 bg-amber-50 rounded-xl text-amber-800 text-sm border border-amber-100">Cadastre membros na aba <strong>Família</strong> para adicioná-los aqui.</div>) : (<div className="grid grid-cols-2 gap-3">{familyMembers.map(member => { const isSelected = newTripParticipants.includes(member.id); return (<div key={member.id} onClick={() => toggleParticipant(member.id)} className={`p-3 rounded-xl border cursor-pointer flex items-center justify-between transition-all ${isSelected ? 'bg-violet-700 text-white border-violet-700 shadow-md shadow-violet-200' : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-50'}`}><span className={`font-bold text-sm ${isSelected ? 'text-white' : 'text-slate-900'}`}>{member.name}</span>{isSelected && <Check className="w-4 h-4" />}</div>); })}</div>)}</div>
    ...
    if (selectedTrip) {
        ...
        return (
            <div className="space-y-6 animate-in fade-in zoom-in duration-300 pb-24">
                    ...
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                            ...
                            <div className="divide-y divide-slate-100">
                                {tripTransactions.length === 0 ? <div className="p-8 text-center text-slate-500"><Sparkles className="w-8 h-8 text-violet-300 mx-auto mb-2" /><p className="text-sm font-medium">Nenhuma despesa registrada ainda.</p><p className="text-xs text-slate-400">Comece a aproveitar sua viagem!</p></div> : 
                                    tripTransactions.map(t => (
                                        <div key={t.id} className="flex justify-between items-center p-4 hover:bg-slate-50 transition-colors">
                                            <div className="flex items-center gap-3"><div className="h-10 w-10 rounded-full bg-violet-50 flex items-center justify-center text-violet-700 font-bold border border-violet-100">{getCategoryIcon(t.category)({ className: "w-5 h-5" })}</div><div><p className="text-sm font-bold text-slate-900">{t.description}</p><div className="flex gap-2 text-xs text-slate-500"><span>{new Date(t.date).toLocaleDateString('pt-BR')}</span><span>•</span><span>{t.category}</span></div></div></div><span className="font-bold text-slate-800">{formatCurrency(t.amount, selectedTrip.currency)}</span>
                                        </div>
                                    ))
                                }
                            </div>
                ...
                {activeTab === 'ITINERARY' && (
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                        <Card title={editingItineraryId ? "Editar Evento" : "Adicionar Evento"} className="no-print">
                            <div className="space-y-3"><div className="flex flex-col gap-2"><div className="flex gap-2"><div className="relative flex-1"><div className="bg-white border border-slate-300 rounded-xl p-2.5 flex items-center gap-2 shadow-sm hover:border-violet-400 transition-all focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-100 relative h-12 overflow-hidden"><div className="absolute top-3 left-3 flex items-center gap-2 pointer-events-none"><Calendar className="w-4 h-4 text-violet-600" /></div><input type="date" className="w-full h-full pl-8 pr-2 bg-transparent text-sm font-bold text-slate-900 border-none outline-none cursor-pointer" value={itiDate} onClick={(e) => e.currentTarget.showPicker()} onChange={e => setItiDate(e.target.value)} /></div></div><input type="time" className="w-24 rounded-xl border border-slate-300 p-2 text-sm focus:ring-2 focus:ring-violet-500 outline-none bg-white text-slate-900 font-bold" value={itiTime} onChange={e => setItiTime(e.target.value)} /></div><select className="w-full rounded-xl border border-slate-300 p-2.5 text-sm focus:ring-2 focus:ring-violet-500 outline-none bg-white text-slate-900 font-bold" value={itiType} onChange={e => setItiType(e.target.value as any)}><option value="FLIGHT">Voo</option><option value="LODGING">Hospedagem</option><option value="ACTIVITY">Passeio</option><option value="FOOD">Comida</option><option value="OTHER">Outro</option></select></div><div className="flex gap-2"><input className="flex-1 rounded-xl border border-slate-300 p-2 text-sm focus:ring-2 focus:ring-violet-500 outline-none bg-white text-slate-900 font-bold placeholder-slate-500" placeholder="Descrição (ex: Voo LATAM, Jantar)" value={itiDesc} onChange={e => setItiDesc(e.target.value)} /><Button onClick={handleSaveItineraryItem} disabled={!itiDate || !itiDesc} size="sm">{editingItineraryId ? <Save className="w-4 h-4" /> : <Plus className="w-4 h-4" />}</Button>{editingItineraryId && (<Button onClick={() => { setEditingItineraryId(null); setItiDesc(''); setItiTime(''); }} size="sm" variant="secondary"><X className="w-4 h-4" /></Button>)}</div></div>
                        </Card>
                        <div className="relative border-l-2 border-violet-100 ml-4 space-y-6">
                            ...
                            {selectedTrip.itinerary?.sort((a, b) => (a.date + (a.time || '')) > (b.date + (b.time || '')) ? 1 : -1).map(item => (
                                <div key={item.id} className={`relative pl-6 ${editingItineraryId === item.id ? 'opacity-50' : ''}`}><div className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-2 border-white shadow-sm ${item.type === 'FLIGHT' ? 'bg-blue-500' : item.type === 'LODGING' ? 'bg-indigo-500' : item.type === 'FOOD' ? 'bg-orange-500' : 'bg-emerald-500'}`}></div><div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex justify-between items-start"><div><div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{new Date(item.date).toLocaleDateString('pt-BR')} {item.time ? `• ${item.time}` : ''}</span><span className={`text-[9px] px-1.5 py-0.5 rounded text-white font-bold ${item.type === 'FLIGHT' ? 'bg-blue-500' : item.type === 'LODGING' ? 'bg-indigo-500' : item.type === 'FOOD' ? 'bg-orange-500' : 'bg-emerald-500'}`}>{item.type}</span></div><p className="font-bold text-slate-800 text-sm">{item.description}</p>{item.location && <p className="text-xs text-slate-600 flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3" /> {item.location}</p>}</div><div className="flex gap-1 no-print"><button onClick={() => startEditingItinerary(item)} className="p-1 text-slate-400 hover:text-violet-600"><Pencil className="w-4 h-4" /></button><button onClick={() => deleteItineraryItem(item.id)} className="p-1 text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button></div></div></div>
                            ))}
                        </div>
                    </div>
                )}
                ...
                {/* TAB 3: CHECKLIST */}
                {activeTab === 'CHECKLIST' && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300"><div className="flex gap-2 no-print"><input className="flex-1 rounded-xl border border-slate-300 p-3 shadow-sm focus:ring-2 focus:ring-violet-500 outline-none bg-white text-slate-900 font-bold placeholder-slate-500" placeholder={editingChecklistId ? "Editar item..." : "Adicionar item (ex: Passaporte, Protetor solar)"} value={checkItem} onChange={e => setCheckItem(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSaveChecklistItem()} /><Button onClick={handleSaveChecklistItem}>{editingChecklistId ? <Save className="w-5 h-5" /> : <Plus className="w-5 h-5" />}</Button>{editingChecklistId && (<Button onClick={() => { setEditingChecklistId(null); setCheckItem(''); }} variant="secondary"><X className="w-5 h-5" /></Button>)}</div><div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden divide-y divide-slate-100">{(!selectedTrip.checklist || selectedTrip.checklist.length === 0) && (<div className="p-8 text-center text-slate-500"><ListChecks className="w-8 h-8 text-slate-300 mx-auto mb-2" /><p className="text-sm">Sua lista está vazia.</p></div>)}{selectedTrip.checklist?.map(item => (<div key={item.id} className={`p-4 flex items-center justify-between hover:bg-slate-50 transition-colors ${editingChecklistId === item.id ? 'bg-violet-50' : ''}`}><div className="flex items-center gap-3 flex-1"><button onClick={() => toggleChecklistItem(item.id)} className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${item.isCompleted ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300'}`}>{item.isCompleted && <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />}</button><span className={`text-sm font-medium ${item.isCompleted ? 'text-slate-400 line-through' : 'text-slate-900'}`}>{item.text}</span></div><div className="flex gap-2 no-print"><button onClick={() => startEditingChecklist(item)} className="text-slate-300 hover:text-violet-600"><Pencil className="w-4 h-4" /></button><button onClick={() => deleteChecklistItem(item.id)} className="text-slate-300 hover:text-red-500"><X className="w-4 h-4" /></button></div></div>))}</div></div>)}
                ...
                {/* TAB 5: SHOPPING */}
                {activeTab === 'SHOPPING' && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300"><Card title={editingShoppingId ? "Editar Item" : "Lista de Desejos"}><div className="bg-violet-50 p-4 rounded-xl mb-4 border border-violet-100 flex justify-between items-center"><span className="text-sm font-bold text-violet-700">Previsão Total de Gastos</span><span className="text-lg font-black text-violet-900">{formatCurrency((selectedTrip.shoppingList || []).reduce((acc, item) => acc + (item.estimatedCost || 0), 0), selectedTrip.currency)}</span></div><div className="flex gap-2 mb-4 no-print"><input className="flex-[2] rounded-xl border border-slate-300 p-3 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-violet-500 outline-none" placeholder="Item (ex: iPhone, Perfume)" value={shopItem} onChange={e => setShopItem(e.target.value)} /><input type="number" className="flex-1 rounded-xl border border-slate-300 p-3 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-violet-500 outline-none" placeholder="Valor Est." value={shopEstCost} onChange={e => setShopEstCost(e.target.value)} /><Button onClick={handleSaveShoppingItem} disabled={!shopItem}>{editingShoppingId ? <Save className="w-5 h-5" /> : <Plus className="w-5 h-5" />}</Button>{editingShoppingId && (<Button onClick={() => { setEditingShoppingId(null); setShopItem(''); setShopEstCost(''); }} variant="secondary"><X className="w-5 h-5" /></Button>)}</div><div className="space-y-2">{(!selectedTrip.shoppingList || selectedTrip.shoppingList.length === 0) && (<div className="p-8 text-center text-slate-500"><ShoppingBag className="w-8 h-8 text-slate-300 mx-auto mb-2" /><p className="text-sm">Lista vazia.</p></div>)}{selectedTrip.shoppingList?.map(item => (<div key={item.id} className={`flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-200 ${editingShoppingId === item.id ? 'ring-2 ring-violet-200' : ''}`}><div className="flex items-center gap-3 flex-1"><button onClick={() => toggleShoppingItem(item.id)} className={`w-5 h-5 rounded border flex items-center justify-center ${item.purchased ? 'bg-emerald-500 border-emerald-500' : 'border-slate-400 bg-white'}`}>{item.purchased && <Check className="w-3 h-3 text-white" />}</button><div><p className={`font-bold text-sm ${item.purchased ? 'text-slate-400 line-through' : 'text-slate-800'}`}>{item.item}</p>{item.estimatedCost ? <p className="text-xs text-slate-500">Est: {formatCurrency(item.estimatedCost, selectedTrip.currency)}</p> : null}</div></div><div className="flex gap-2 no-print"><button onClick={() => startEditingShopping(item)} className="text-slate-300 hover:text-violet-600"><Pencil className="w-4 h-4" /></button><button onClick={() => deleteShoppingItem(item.id)} className="text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button></div></div>))}</div></Card></div>)}

                {/* TAB 6: EXCHANGE */}
                {activeTab === 'EXCHANGE' && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300"><Card title={editingExchangeId ? "Editar Câmbio" : "Controle de Câmbio"}><div className="bg-violet-50 p-4 rounded-xl mb-6 border border-violet-100"><div className="flex justify-between items-center"><div><p className="text-xs font-bold text-violet-600 uppercase">Média Ponderada</p><h3 className="text-2xl font-black text-violet-900">{(() => { const entries = selectedTrip.exchangeEntries || []; const totalBRL = entries.reduce((acc, e) => acc + e.amountBRL, 0); const totalForeign = entries.reduce((acc, e) => acc + e.amountForeign, 0); const avg = totalForeign > 0 ? totalBRL / totalForeign : 0; return `R$ ${avg.toFixed(2)}`; })()}</h3></div><div className="text-right"><p className="text-xs font-bold text-violet-600 uppercase">Total Comprado</p><p className="text-lg font-bold text-violet-800">{formatCurrency((selectedTrip.exchangeEntries || []).reduce((acc, e) => acc + e.amountForeign, 0), selectedTrip.currency)}</p></div></div></div><div className="flex flex-col gap-3 mb-6 bg-white p-4 rounded-xl border border-slate-200 shadow-sm no-print"><h4 className="font-bold text-sm text-slate-700">{editingExchangeId ? "Editar Entrada" : "Nova Compra de Moeda"}</h4><div className="flex gap-2"><input type="date" className="w-32 p-2 border rounded-lg text-sm" value={exchangeDate} onClick={(e) => e.currentTarget.showPicker()} onChange={e => setExchangeDate(e.target.value)} /><input type="number" className="flex-1 p-2 border rounded-lg text-sm" placeholder="Valor em R$ (BRL)" value={exchangeBRL} onChange={e => setExchangeBRL(e.target.value)} /><input type="number" className="flex-1 p-2 border rounded-lg text-sm" placeholder={`Valor em ${selectedTrip.currency}`} value={exchangeForeign} onChange={e => setExchangeForeign(e.target.value)} /></div><div className="flex gap-2"><Button onClick={handleSaveExchangeEntry} disabled={!exchangeBRL || !exchangeForeign} className="w-full">{editingExchangeId ? <><Save className="w-4 h-4 mr-2" /> Salvar</> : <><Plus className="w-4 h-4 mr-2" /> Registrar Câmbio</>}</Button>{editingExchangeId && (<Button onClick={() => { setEditingExchangeId(null); setExchangeBRL(''); setExchangeForeign(''); }} variant="secondary">Cancelar</Button>)}</div></div><div className="space-y-2">{selectedTrip.exchangeEntries?.map(entry => (<div key={entry.id} className={`flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-200 text-sm ${editingExchangeId === entry.id ? 'ring-2 ring-violet-200' : ''}`}><div><p className="font-bold text-slate-800">{new Date(entry.date).toLocaleDateString('pt-BR')}</p><p className="text-xs text-slate-500">Taxa: R$ {entry.exchangeRate.toFixed(4)}</p></div><div className="text-right"><p className="font-bold text-emerald-600">+ {formatCurrency(entry.amountForeign, entry.currency)}</p><p className="text-xs text-slate-500">- {formatCurrency(entry.amountBRL, 'BRL')}</p></div><div className="flex gap-2 ml-4 no-print"><button onClick={() => startEditingExchange(entry)} className="text-slate-300 hover:text-violet-600"><Pencil className="w-4 h-4" /></button><button onClick={() => deleteExchangeEntry(entry.id)} className="text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button></div></div>))}</div></Card></div>)}

            </div>
        );
    }

    // ... (Keep trips list view exactly as is)
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20">
            <div className="flex justify-between items-center"><div><h2 className="text-2xl font-bold text-slate-800">Minhas Viagens</h2><p className="text-slate-600 text-sm">Gerencie orçamentos e roteiros.</p></div><Button onClick={() => setIsCreatingTrip(true)} className="rounded-xl shadow-md shadow-emerald-500/20"><Plus className="w-4 h-4 mr-2" /> Nova Viagem</Button></div>
            {trips.length === 0 && (<div className="text-center py-12 bg-white rounded-2xl border border-dashed border-slate-300"><div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-400"><Plane className="w-8 h-8" /></div><h3 className="text-lg font-bold text-slate-700">Nenhuma viagem encontrada</h3><p className="text-slate-500 text-sm mb-4">Crie sua primeira viagem para começar a organizar.</p><Button onClick={() => setIsCreatingTrip(true)} variant="secondary">Criar Viagem Agora</Button></div>)}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{trips.map(trip => (<div key={trip.id} className="group relative bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl transition-all cursor-pointer transform hover:-translate-y-1" onClick={() => setSelectedTripId(trip.id)}><div className="h-40 bg-slate-200 relative overflow-hidden"><img src={trip.imageUrl || `https://picsum.photos/seed/${trip.id}/500/300`} alt={trip.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" /><div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 to-transparent opacity-80" /><span className="absolute top-3 right-3 bg-white/20 backdrop-blur-md px-2 py-1 rounded-md text-xs font-bold text-white shadow-sm border border-white/20">{trip.currency}</span><div className="absolute bottom-3 left-4 text-white"><h3 className="font-bold text-lg drop-shadow-md">{trip.name}</h3><p className="text-xs text-slate-300 flex items-center mt-1"><Calendar className="w-3 h-3 mr-1" />{new Date(trip.startDate).toLocaleDateString('pt-BR')}</p></div></div><div className="p-5"><div className="flex items-center justify-between"><div className="flex -space-x-2 pl-1">{trip.participants.slice(0, 4).map(p => (<div key={p.id} className="h-8 w-8 rounded-full ring-2 ring-white bg-violet-100 flex items-center justify-center text-violet-700 text-xs font-bold uppercase shadow-sm" title={p.name}>{p.name[0]}</div>))}{trip.participants.length > 4 && (<div className="h-8 w-8 rounded-full ring-2 ring-white bg-slate-100 flex items-center justify-center text-slate-600 text-slate-600 text-xs font-bold uppercase shadow-sm">+{trip.participants.length - 4}</div>)}</div><span className="text-violet-600 font-bold text-xs bg-violet-50 px-3 py-1.5 rounded-full group-hover:bg-violet-600 group-hover:text-white transition-colors">ABRIR</span></div></div></div>))}</div>
        </div>
    );
};