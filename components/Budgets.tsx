    ...
    return (
        <div className="space-y-6 pb-24 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Orçamentos Inteligentes</h2>
                    <p className="text-slate-600 dark:text-slate-400 text-sm">Planeje seus gastos e economize mais.</p>
                </div>
                ...
                <Card className="bg-slate-50/50 border-slate-200" title={editingBudget ? "Editar Orçamento" : "Novo Orçamento"}>
                    <form onSubmit={handleSave} className="space-y-4">
                        <div>
                            ...
                            <select
                                className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-slate-900 dark:text-white"
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Limite Mensal</label>
                            <input
                                type="number"
                                step="0.01"
                                className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-slate-900 dark:text-white"
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            />
                            <label htmlFor="rollover" className="text-sm font-medium text-slate-700 dark:text-slate-300">
            ...
            <div className="grid grid-cols-1 gap-4">
                {budgets.map(budget => {
                    ...
                    return (
                        <div key={budget.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm relative overflow-hidden group">
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-800 dark:text-white">{budget.categoryId}</h3>
                ...
                {budgets.length === 0 && !isFormOpen && (
                    <div className="text-center py-10 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                        <div className="bg-white dark:bg-slate-800 p-4 rounded-full inline-flex mb-3 shadow-sm">
                            <TrendingUp className="w-8 h-8 text-slate-400" />
                        </div>
                        <p className="text-slate-600 dark:text-slate-400 font-medium">Nenhum orçamento definido.</p>
