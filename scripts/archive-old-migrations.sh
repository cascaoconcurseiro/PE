#!/bin/bash
# Script para arquivar migrations antigas (já consolidadas)
# Execute com cuidado: verifique quais migrations já foram aplicadas no banco

echo "📦 Arquivando Migrations Antigas"
echo "================================"
echo ""

# Criar diretório de arquivo
ARCHIVE_DIR="supabase/migrations/archive/2026-01-27_consolidacao"
mkdir -p "$ARCHIVE_DIR"

# Migrations que podem ser arquivadas (anteriores à consolidação)
...
echo "📦 Movendo migrations para arquivo..."
for migration in "${MIGRATIONS_TO_ARCHIVE[@]}"; do
    if [ -f "supabase/migrations/$migration" ]; then
        mv "supabase/migrations/$migration" "$ARCHIVE_DIR/$migration"
        echo "  ✓ Arquivado: $migration"
    fi
done

echo ""
echo "✅ Migrations arquivadas em: $ARCHIVE_DIR"
echo ""
echo "⚠️  IMPORTANTE:"
echo "   - Essas migrations JÁ FORAM APLICADAS no banco"
echo "   - Não delete do banco, apenas arquive no sistema de arquivos"
echo "   - Mantenha apenas migrations essenciais na pasta principal"
