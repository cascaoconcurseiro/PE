...
export const calculateProjectedBalance = (
    ...
    transactions.forEach(t => {
        ...
        // Nota: Num sistema real, teríamos um flag "isPaid". Aqui assumimos data futura = pendente.
        if (tDate >= today) {
            // Ignorar transferências internas, pois não alteram o patrimônio líquido total
            if (t.type === TransactionType.TRANSFER) return;

            const amountBRL = convertToBRL(t.amount, 'BRL'); // Simplificação: assume BRL base ou converte
