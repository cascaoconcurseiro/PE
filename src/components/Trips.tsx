    ...
    if (selectedTrip) {
        ...
        return (
            <div className="space-y-6 animate-in fade-in zoom-in duration-300 pb-24">
                ...
                <Card className="bg-gradient-to-br from-violet-700 to-indigo-800 text-white border-none shadow-lg relative overflow-hidden print:bg-white print:text-black print:border print:border-black">
                ...
                <div className="flex p-1 bg-slate-200 rounded-xl overflow-x-auto no-scrollbar no-print">
                    {['OVERVIEW', 'ITINERARY', 'CHECKLIST', 'STATS', 'SHOPPING', 'EXCHANGE'].map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab as any)} className={`flex-1 min-w-[80px] py-2 text-xs font-bold rounded-lg transition-all ${activeTab === tab ? 'bg-white text-violet-800 shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}>
                            {tab === 'OVERVIEW' ? 'Gastos' : tab === 'ITINERARY' ? 'Roteiro' : tab === 'CHECKLIST' ? 'Checklist' : tab === 'STATS' ? 'Relatórios' : tab === 'SHOPPING' ? 'Compras' : 'Câmbio'}
                        </button>
                    ))}
                </div>
                ...
                {/* Other Tabs: Simplified for brevity but fully functional based on original code */}
                {activeTab === 'ITINERARY' && (
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="relative border-l-2 border-violet-100 ml-4 space-y-6">
                            {selectedTrip.itinerary?.sort((a, b) => (a.date + (a.time || '')) > (b.date + (b.time || '')) ? 1 : -1).map(item => (
                                <div key={item.id} className={`relative pl-6 ${editingItineraryId === item.id ? 'opacity-50' : ''}`}><div className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-2 border-white shadow-sm ${item.type === 'FLIGHT' ? 'bg-blue-500' : item.type === 'LODGING' ? 'bg-indigo-500' : item.type === 'FOOD' ? 'bg-orange-500' : 'bg-emerald-500'}`}></div><div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex justify-between items-start"><div><div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{new Date(item.date).toLocaleDateString('pt-BR')} {item.time ? `• ${item.time}` : ''}</span><span className={`text-[9px] px-1.5 py-0.5 rounded text-white font-bold ${item.type === 'FLIGHT' ? 'bg-blue-500' : item.type === 'LODGING' ? 'bg-indigo-500' : item.type === 'FOOD' ? 'bg-orange-500' : 'bg-emerald-500'}`}>{item.type}</span></div><p className="font-bold text-slate-800 text-sm">{item.description}</p></div><div className="flex gap-1 no-print"><button onClick={() => startEditingItinerary(item)} className="p-1 text-slate-400 hover:text-violet-600"><Pencil className="w-4 h-4" /></button><button onClick={() => deleteItineraryItem(item.id)} className="p-1 text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button></div></div></div>
                            ))}
                        </div>
                    </div>
                )}
                {activeTab === 'CHECKLIST' && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300"><div className="flex gap-2 no-print"><input className="flex-1 rounded-xl border border-slate-300 p-3 shadow-sm focus:ring-2 focus:ring-violet-500 outline-none bg-white text-slate-900 font-bold placeholder-slate-500" placeholder="Adicionar item..." value={checkItem} onChange={e => setCheckItem(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSaveChecklistItem()} /><Button onClick={handleSaveChecklistItem}>{editingChecklistId ? <Save className="w-5 h-5" /> : <Plus className="w-5 h-5" />}</Button></div><div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden divide-y divide-slate-100">{selectedTrip.checklist?.map(item => (<div key={item.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors"><div className="flex items-center gap-3 flex-1"><button onClick={() => toggleChecklistItem(item.id)} className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${item.isCompleted ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300'}`}>{item.isCompleted && <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />}</button><span className={`text-sm font-medium ${item.isCompleted ? 'text-slate-400 line-through' : 'text-slate-900'}`}>{item.text}</span></div><div className="flex gap-2 no-print"><button onClick={() => startEditingChecklist(item)} className="text-slate-300 hover:text-violet-600"><Pencil className="w-4 h-4" /></button><button onClick={() => deleteChecklistItem(item.id)} className="text-slate-300 hover:text-red-500"><X className="w-4 h-4" /></button></div></div>))}</div></div>)}
                {activeTab === 'STATS' && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300"><Card title="Gastos por Categoria">{tripTransactions.length > 0 ? (<div className="h-64"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={Object.entries(tripTransactions.reduce((acc, t) => { if (t.type === TransactionType.EXPENSE) acc[t.category] = (acc[t.category] || 0) + t.amount; return acc; }, {} as any)).map(([name, value]) => ({ name, value }))} dataKey="value" cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5}>{Object.entries(tripTransactions).map((_, index) => (<Cell key={`cell-${index}`} fill={['#8b5cf6', '#ec4899', '#f59e0b', '#10b981', '#3b82f6'][index % 5]} />))}</Pie><Tooltip formatter={(value: number) => formatCurrency(value)} /><Legend /></PieChart></ResponsiveContainer></div>) : (<div className="h-32 flex items-center justify-center text-slate-500 text-sm">Sem dados suficientes</div>)}</Card></div>)}
                {activeTab === 'SHOPPING' && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300"><Card title="Lista de Desejos"><div className="flex gap-2 mb-4 no-print"><input className="flex-[2] rounded-xl border border-slate-300 p-3 text-sm font-bold text-slate-900" placeholder="Item..." value={shopItem} onChange={e => setShopItem(e.target.value)} /><input type="number" className="flex-1 rounded-xl border border-slate-300 p-3 text-sm font-bold text-slate-900" placeholder="Valor" value={shopEstCost} onChange={e => setShopEstCost(e.target.value)} /><Button onClick={handleSaveShoppingItem}><Plus className="w-5 h-5" /></Button></div><div className="space-y-2">{selectedTrip.shoppingList?.map(item => (<div key={item.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-200"><div className="flex items-center gap-3 flex-1"><button onClick={() => toggleShoppingItem(item.id)} className={`w-5 h-5 rounded border flex items-center justify-center ${item.purchased ? 'bg-emerald-500 border-emerald-500' : 'border-slate-400 bg-white'}`}>{item.purchased && <Check className="w-3 h-3 text-white" />}</button><div><p className="font-bold text-sm">{item.item}</p>{item.estimatedCost ? <p className="text-xs text-slate-500">{formatCurrency(item.estimatedCost, selectedTrip.currency)}</p> : null}</div></div><div className="flex gap-2 no-print"><button onClick={() => startEditingShopping(item)}><Pencil className="w-4 h-4" /></button><button onClick={() => deleteShoppingItem(item.id)}><Trash2 className="w-4 h-4" /></button></div></div>))}</div></Card></div>)}
                {activeTab === 'EXCHANGE' && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300"><Card title="Câmbio"><div className="flex flex-col gap-3 mb-6 bg-white p-4 rounded-xl border border-slate-200 shadow-sm no-print"><div className="flex gap-2"><input type="number" className="flex-1 p-2 border rounded-lg text-sm" placeholder="R$" value={exchangeBRL} onChange={e => setExchangeBRL(e.target.value)} /><input type="number" className="flex-1 p-2 border rounded-lg text-sm" placeholder={selectedTrip.currency} value={exchangeForeign} onChange={e => setExchangeForeign(e.target.value)} /></div><Button onClick={handleSaveExchangeEntry} disabled={!exchangeBRL || !exchangeForeign}>Registrar</Button></div><div className="space-y-2">{selectedTrip.exchangeEntries?.map(entry => (<div key={entry.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-200 text-sm"><div><p className="font-bold">{new Date(entry.date).toLocaleDateString('pt-BR')}</p><p className="text-xs text-slate-500">Taxa: R$ {entry.exchangeRate.toFixed(4)}</p></div><div className="text-right"><p className="font-bold text-emerald-600">+ {formatCurrency(entry.amountForeign, entry.currency)}</p><p className="text-xs text-slate-500">- {formatCurrency(entry.amountBRL, 'BRL')}</p></div><div className="flex gap-2 ml-4 no-print"><button onClick={() => startEditingExchange(entry)}><Pencil className="w-4 h-4" /></button><button onClick={() => deleteExchangeEntry(entry.id)}><Trash2 className="w-4 h-4" /></button></div></div>))}</div></Card></div>)}
            </div>
        );
    }
