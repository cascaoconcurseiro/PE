...
import { AnticipateInstallmentsModal } from './transactions/AnticipateInstallmentsModal';
...
interface AccountsProps {
    ...
    showValues: boolean;
    currentDate?: Date;
    onAnticipateInstallments: (ids: string[], date: string, accountId: string) => void;
}
...
export const Accounts: React.FC<AccountsProps> = ({ accounts, transactions, onAddAccount, onUpdateAccount, onDeleteAccount, onAddTransaction, showValues, currentDate = new Date(), onAnticipateInstallments }) => {
    ...
    const handleAnticipateClick = (tx: Transaction) => {
    ...
    const handleConfirmAnticipation = (ids: string[], date: string, accountId: string) => {
        if (onAnticipateInstallments) {
            onAnticipateInstallments(ids, date, accountId);
        }
            ...
            <div className="space-y-6 animate-in slide-in-from-right duration-300 pb-24">
                ...
                {selectedAccount.type === AccountType.CREDIT_CARD ? (
                    <CreditCardDetail 
                        ...
                        onAnticipateInstallments={handleAnticipateClick} 
                    />
                ) : (
                    <BankingDetail 
                        account={selectedAccount} 
            ...
            {activeTab === 'CARDS' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {creditCards.map(account => {
                        ...
                        return (
                            <div key={account.id} onClick={() => handleAccountClick(account)} className="group bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all cursor-pointer relative overflow-hidden min-h-[220px] flex flex-col justify-between">
