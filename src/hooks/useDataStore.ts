...
export const useDataStore = () => {
    ...
    const handleImportData = async (data: any) => {
        ...
        if (!validation.success) {
            ...
            alert("Erro: O arquivo de backup é inválido ou está corrompido. Verifique o console para mais detalhes.");
            return;
        }
        ...
        await db.transaction('rw', [db.accounts, db.transactions, db.trips, db.budgets, db.goals, db.familyMembers, db.assets, db.snapshots], async () => {
            if (validData.accounts) await db.accounts.bulkPut(validData.accounts);
            if (validData.transactions) await db.transactions.bulkPut(validData.transactions);
            if (validData.trips) await db.trips.bulkPut(validData.trips);
            if (validData.budgets) await db.budgets.bulkPut(validData.budgets);
            if (validData.goals) await db.goals.bulkPut(validData.goals);
            if (validData.familyMembers) await db.familyMembers.bulkPut(validData.familyMembers);
            if (validData.assets) await db.assets.bulkPut(validData.assets);
            if (validData.snapshots) await db.snapshots.bulkPut(validData.snapshots);
        });
        if (validData.customCategories) {
            setCustomCategories(validData.customCategories);
