-- ==============================================================================
-- FIX: ESPELHAMENTO E NOTIFICAÇÕES DE TRANSAÇÕES COMPARTILHADAS
-- DATA: 2025-12-19
-- PROBLEMAS:
--   1. Mirrors não estão sendo criados para transações compartilhadas
--   2. Notificações duplicadas para parcelas (10 parcelas = 10 notificações)
-- ==============================================================================

BEGIN;

-- ============================================================================
-- PARTE 1: CORRIGIR FUNÇÃO sync_shared_transaction
-- ============================================================================
...
BEGIN
    ...
    LOOP
        ...
        RAISE NOTICE '[sync_shared_transaction] Member: %, LinkedUser: %', v_target_member_id, v_target_user_id;
        ...
        IF v_target_user_id IS NOT NULL AND v_target_user_id != v_tx_rec.user_id THEN
            ...
            -- Verificar se mirror já existe (evitar duplicatas)
            IF NOT EXISTS (
                ...
                INSERT INTO public.transactions (
                    ...
                    v_tx_rec.id,  -- Referência à transação original
                    '[]'::jsonb,
                    v_target_trip_id,
                    v_tx_rec.currency,
                ...
                RAISE NOTICE '[sync_shared_transaction] Mirror criado para user % - %', v_target_user_id, v_tx_rec.description;
            ELSE
                RAISE NOTICE '[sync_shared_transaction] Mirror já existe para user % - %', v_target_user_id, v_tx_rec.description;
            END IF;
        ELSE
            RAISE NOTICE '[sync_shared_transaction] Membro % sem vínculo ou é o próprio criador', v_target_member_id;
        END IF;
    END LOOP;
END;
$$;
...
-- ============================================================================
-- PARTE 2: CORRIGIR TRIGGER DE NOTIFICAÇÃO (AGRUPAR PARCELAS)
-- ============================================================================
    ...
    IF NEW.is_shared = TRUE AND NEW.shared_with IS NOT NULL AND jsonb_array_length(NEW.shared_with) > 0 THEN
        ...
        IF NEW.is_installment = TRUE AND NEW.current_installment IS NOT NULL THEN
            IF NEW.current_installment > 1 THEN
                -- Não é a primeira parcela, não notificar
                RETURN NEW;
            END IF;
            -- É a primeira parcela, calcular valor total
        ...
        -- Limpar descrição (remover sufixo de parcela se existir)
        ...
        LOOP
            ...
            IF member_user_id IS NOT NULL AND member_user_id != NEW.user_id THEN
                ...
                -- Verificar se já existe notificação recente para esta série (evitar duplicatas)
                IF NEW.series_id IS NOT NULL THEN
                    IF EXISTS (
                        ...
                        -- Já existe notificação para esta série, pular
                        CONTINUE;
                    END IF;
                END IF;
...
-- ============================================================================
-- PARTE 4: ATUALIZAR RPC create_transaction PARA CHAMAR sync_shared_transaction
-- ============================================================================

-- Verificar se a RPC atual chama sync_shared_transaction
-- Se não, precisamos atualizar
...
BEGIN
    ...
    INSERT INTO public.transactions (
        ...
        p_shared_with,
        CASE WHEN p_payer_id = 'me' OR p_payer_id = '' THEN NULL ELSE p_payer_id END,
        p_currency,
        NOW(), NOW()
    ) RETURNING id INTO v_new_id;

    -- ✅ CRÍTICO: Sincronizar transações compartilhadas (criar mirrors)
...
-- ============================================================================
-- PARTE 5: FORÇAR RESYNC DE TRANSAÇÕES COMPARTILHADAS EXISTENTES SEM MIRROR
-- ============================================================================
...
DECLARE
    ...
    v_count INT := 0;
BEGIN
    RAISE NOTICE 'Iniciando resync de transações compartilhadas...';
    ...
    FOR r IN 
        ...
        WHERE t.is_shared = true 
          ...
          -- Apenas transações que não têm mirror
          AND NOT EXISTS (
    ...
    RAISE NOTICE 'Resync concluído. Total: % transações processadas', v_count;
END $$;

-- ============================================================================
-- PARTE 6: LIMPAR NOTIFICAÇÕES DUPLICADAS
-- ============================================================================

-- Manter apenas a notificação mais recente por série
DELETE FROM public.user_notifications n1
WHERE n1.type = 'SHARED_EXPENSE'
  AND n1.metadata->>'seriesId' IS NOT NULL
  AND EXISTS (
      SELECT 1 FROM public.user_notifications n2
      WHERE n2.user_id = n1.user_id
        AND n2.type = 'SHARED_EXPENSE'
...
SELECT 'Mirrors criados' as tipo, COUNT(*) as total
FROM public.transactions
WHERE source_transaction_id IS NOT NULL AND deleted = false;

SELECT 'Notificações SHARED_EXPENSE' as tipo, COUNT(*) as total
