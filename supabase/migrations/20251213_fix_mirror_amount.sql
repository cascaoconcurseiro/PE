...
DECLARE
    ...
    split_amount NUMERIC; -- Variavel para armazenar o valor da divisão
BEGIN
    ...
    LOOP
        ...
        -- FIX: Extrair o valor CORRETO da divisão (assignedAmount)
        -- Se não existir (fallback), usa o total, mas o correto é existir.
        ...
        IF target_user_id IS NOT NULL THEN
            ...
            IF (TG_OP = 'INSERT') THEN
                ...
                -- Verifica se existe transação manual similar na conta destino
                -- Nota: Usamos split_amount para verificar compatibilidade de valor
                SELECT TRUE INTO is_duplicate_check
                FROM transactions t
                WHERE t.user_id = target_user_id
                  AND t.mirror_transaction_id IS NULL -- Só manuais
                  AND t.date = NEW.date
                  AND t.amount BETWEEN (split_amount * 0.99) AND (split_amount * 1.01) -- Tol: 1% do valor da PARTE
                  -- AND similarity(t.description, NEW.description) > 0.6 -- Requer extensão pg_trgm (removido para evitar dependência)
                LIMIT 1;
                ...
                INSERT INTO transactions (
                    ...
                    split_amount, -- <--- USANDO O VALOR DA DIVISÃO
                    NEW.type,
                    NEW.date,
                    sanitized_category,
                    NULL, 
            ...
            ELSIF (TG_OP = 'UPDATE') THEN
                UPDATE transactions
                SET 
                    amount = split_amount, -- <--- ATUALIZANDO COM O NOVO VALOR DA DIVISÃO
                    date = NEW.date,
