-- ==============================================================================
-- MIGRATION: FORCE SYNC SHARED EXPENSES V2 (AUTO-LINK + SYNC)
-- DATE: 2025-12-15
-- DESCRIPTION: 1. Auto-links family members by email if they are not linked.
--              2. Forces sync of shared past transactions.
-- ==============================================================================
...
BEGIN
    RAISE NOTICE 'STARTING FORCE SYNC V2...';

    -- 1. FIX FAMILY LINKS (CRITICAL STEP)
    -- If the user was invited but 'linked_user_id' is NULL, we try to match by email now.
    ...
    RAISE NOTICE 'Family Links Fixed/Restored: %', links_fixed;


    -- 2. Loop through ALL Family Members that are now linked
    ...
    LOOP
        ...
        -- 3. Find transactions shared with this member (that are NOT yet synced)
        FOR tx_rec IN 
            SELECT t.* 
