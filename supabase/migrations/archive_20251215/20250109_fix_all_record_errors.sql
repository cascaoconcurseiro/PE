-- MIGRATION: FIX ALL RECORD ERRORS (GLOBAL FIX)
-- DATE: 2025-01-09
-- DESCRIPTION: Fixes 'operator does not exist: record ->> unknown' in ALL functions.
--              We rewrite loops to use Integer Indexing instead of SET RETURNING functions.

-- 1. FIX TRIPS TRIGGER (handle_trip_sharing)
...
BEGIN
    ...
    -- Helper integer loop to avoid RECORD issues
...
-- 2. FIX TRANSACTIONS TRIGGER (handle_mirror_shared_transaction)
-- Even though the error appeared on Trip Edit, this function had the same buggy loop structure.
...
BEGIN
    IF (TG_OP = 'INSERT') AND (auth.uid() = NEW.user_id) AND (NEW.is_shared = true) AND (NEW.shared_with IS NOT NULL) AND (jsonb_array_length(NEW.shared_with) > 0) THEN
        ...
        FOR i IN 0 .. (len - 1)
        LOOP
            split := NEW.shared_with->i; -- Safe extraction
