-- RUN THIS SCRIPT TO APPLY ALL ARCHITECTURAL FIXES
-- This consolidates: Auto-Share Triggers + RPC Fixes + RLS

--------------------------------------------------------------------------------
-- 1. FIX RPC V2 (Ambiguity 400 Error)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 2. AUTO-SHARE TRIGGERS (Enforce Shared Logic)
--------------------------------------------------------------------------------
...
BEGIN
    SELECT EXISTS (
        ...
        AND (
            fm.linked_user_id = NEW.invited_user_id 
            OR 
            up.id = NEW.invited_user_id 
        )
    ) INTO v_is_linked;
...
BEGIN
    IF NEW.status = 'ACCEPTED' THEN
        ...
        INSERT INTO transactions (
            description, amount, type, date, category, account_id, user_id, is_shared, trip_id, observation, currency, exchange_rate, created_at, updated_at
        ) VALUES (
...
DROP POLICY IF EXISTS "Users can view relevant requests" ON shared_transaction_requests;
DROP POLICY IF EXISTS "Users can create requests" ON shared_transaction_requests;
DROP POLICY IF EXISTS "Users can update relevant requests" ON shared_transaction_requests;

CREATE POLICY "Users can view relevant requests" ON shared_transaction_requests FOR SELECT
USING (auth.uid() = requester_id OR auth.uid() = invited_user_id);

CREATE POLICY "Users can create requests" ON shared_transaction_requests FOR INSERT
WITH CHECK (auth.uid() = requester_id);

CREATE POLICY "Users can update relevant requests" ON shared_transaction_requests FOR UPDATE
USING (auth.uid() = requester_id OR auth.uid() = invited_user_id);
