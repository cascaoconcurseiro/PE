-- MIGRATION: NUCLEAR TRIGGER CLEANUP & FIX
-- DATE: 2025-01-09
-- DESCRIPTION: Drops ALL related triggers to ensure no zombie triggers exist.
--              Re-applies the V4 Safe Function Fix (Integer Loop).
--              Re-creates triggers.

-- 1. DROP ALL POTENTIAL TRIGGERS (Known & Zombie names)
...
-- 2. RE-DEFINE SAFE FUNCTIONS (Integer Loop Strategy)
...
-- 3. RE-CREATE TRIGGERS STRICTLY
CREATE TRIGGER on_trip_change AFTER INSERT OR UPDATE ON public.trips FOR EACH ROW EXECUTE FUNCTION handle_trip_sharing();
